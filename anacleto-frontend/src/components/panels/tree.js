...
function Tree({ id, context, panelContext, ...props }) {
	...
	useEffect(() => {
		...
		if (props.events.onLoad) {
			props.events.onLoad.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(nodes);
		}
	...
	const onExpand = useCallback((_event) => {
		if (props.events?.onExpand) {
			props.events?.onExpand.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(_event);
		}
	}, [props.events, context, panelContext, panelsContext]);
	const onCollapse = useCallback((_event) => {
		if (props.events?.onCollapse) {
			props.events.events.onCollapse.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(_event);
		}
	}, [props.events, context, panelContext, panelsContext]);
	const onSelect = useCallback((_event) => {
		if (props.events?.onSelect) {
			props.events.onSelect.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(_event);
		}
	}, [props.events, context, panelContext, panelsContext]);
	const onUnselect = useCallback((_event) => {
		if (props.events.onUnselect) {
			props.events?.onUnselect.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(_event);
		}
	}, [props.events, context, panelContext, panelsContext]);
	...
	const onContextMenu = useCallback((_event) => {
		...
		if (props.events.onContextMenu) {
			contextMenuItem = props.events.onContextMenu.bind({ panel: props, context, components:panelsContext, updatePanelContext, ...panelContext })(_event);
		}
